    ...
    mnInitialFrameId(0), mbCreatedMap(false), mnFirstFrameId(0), mpCamera2(nullptr)
{
    ...
    // Step 1 从配置文件中加载相机参数，ORB参数，IMU参数
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(bOK || mState==RECENTLY_LOST)
        {
            ...
            printf("是否插入关键帧的条件判断： bNeedKF: %d, bOK: %d, mState: %d, mSensor: %d, mbOnlyTracking: %d\n", bNeedKF, bOK, mState, mSensor, mbOnlyTracking);
...
void Tracking::StereoInitialization()
{
    ...
    if(mCurrentFrame.N>500)
    {
        ...
        Verbose::PrintMess("在局部地图中添加该初始关键帧...", Verbose::VERBOSITY_QUIET);
        // 在局部地图中添加该初始关键帧
        mpLocalMapper->InsertKeyFrame(pKFini);

        Verbose::PrintMess("更新当前帧为上一帧...", Verbose::VERBOSITY_QUIET);
        // 更新当前帧为上一帧
        mLastFrame = Frame(mCurrentFrame);
        ...
        mpMapDrawer->SetCurrentCameraPose(mCurrentFrame.mTcw);
        Verbose::PrintMess("追踪成功", Verbose::VERBOSITY_QUIET);
        //追踪成功
        mState=OK;
    }
}
...
bool Tracking::NeedNewKeyFrame()
{
    ...
    printf("c1c参数相关： mnMatchesInliers: %d, nRefMatches*0.25: %f,thRefRatio: %f, bNeedToInsertClose: %d\n",mnMatchesInliers,nRefMatches*0.25,thRefRatio,bNeedToInsertClose);
    ...
    // 相比ORB-SLAM2多了c3,c4
    printf("NeedNewKeFrame:条件判断： c1a: %d; c1b: %d, c1c: %d, c2: %d, c3: %d, c4: %d, bLocalMappingIdle: %d, mpLocalMapper->KeyframesInQueue(): %d\n",c1a,c1b,c1c,c2,c3,c4,bLocalMappingIdle,mpLocalMapper->KeyframesInQueue());
