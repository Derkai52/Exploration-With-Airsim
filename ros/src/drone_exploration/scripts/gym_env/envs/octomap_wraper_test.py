...
def main():
    ...
    for i in range(100):
        ...
        #print("mask",mask)
        print("pcd.shape:", type(pcd[nonnan]), pcd[nonnan].shape, pcd[nonnan].dtype)
        octree.insertPointCloud(
            pointcloud=pcd[nonnan],
            #origin=np.array([img_response.camera_position.x_val, img_response.camera_position.y_val, img_response.camera_position.z_val], dtype=float),
            origin=img_response.camera_position.to_numpy_array().astype(float),
            maxrange=2,
        )
        print(time.time())
