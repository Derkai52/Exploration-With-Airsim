...
class DroneExploreEnvParallel():
    def __init__(self, envs, observation_space=None, action_space=None, copy=True):
        ...
        self.reset_num = 0
        self.action_space_len = self.single_action_space.n
    
    def reset(self):
        observations = []
        for i in range(self.num_envs):
            observations.append([self.envs[i].reset(self.reset_num)])
            self.reset_num += 1
    ...
    def step(self,actions):
        ...
        for i, (env, action) in enumerate(zip(self.envs, actions)):
            ...
            infos.append(info)
        return np.array(observations), np.copy(self._rewards), np.copy(self._dones), infos
